...
import { useApplications, useUpdateApplication } from "@/lib/admin/react-query/applications.hooks";
                      ...
                      <Card key={candidate.id} className="border bg-background p-4 shadow-sm hover:shadow-md transition-shadow relative">
                        <div className="space-y-3">
                          ...
                          <div className="flex items-center justify-between pt-2 border-t gap-2">
                            <Button asChild variant="ghost" size="sm" className="w-full text-xs font-semibold h-7">
  <Link href={`/admin/dashboard/applications/${candidate.id}`} className="flex-1">
                                <Eye className="h-3.5 w-3.5 mr-1" /> View Profile
                              </Link>
</Button>

                                                          {candidate.status === "Offer Sent" && canMove && (
                                <HireDialog 
                                  applicationId={candidate.id!}
                                  candidateName={`${candidate.firstName} ${candidate.lastName}`}
                                  candidateEmail={candidate.email}
                                  candidatePhone={candidate.phone}
                                  defaultPosition={candidate.jobTitle}
                                />
                              )}
                            {canMove && (
                              <Select

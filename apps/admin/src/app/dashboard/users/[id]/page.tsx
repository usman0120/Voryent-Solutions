...
import { useUser } from "@/lib/react-query/users.hooks";
import { useAuth } from "@/providers/auth-provider";
import { use } from "react";
import Link from "next/link";
import { Card, CardHeader, CardTitle, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { UserCard } from "@/components/operations/user-card";
...
export default function UserProfilePage({ params }: UserProfilePageProps) {
  ...
  const canManage = (currentUser as any)?.role === "Super Admin" || (currentUser as any)?.role === "Founder" || (currentUser as any)?.role === "HR" || currentUser?.uid === userProfile?.uid;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <Link href="/dashboard/users" className="text-muted-foreground hover:text-foreground flex items-center gap-1 text-sm font-medium">

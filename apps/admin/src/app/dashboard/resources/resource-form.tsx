...
import { type Resource, resourcesService } from "@/lib/services/resources.service";
...
export function ResourceForm({ initialData, id }: ResourceFormProps) {
  ...
  const onSubmit = async (values: ResourceFormValues) => {
    try {
      ...
      if (id) {
        await resourcesService.update(id, values);
        toast({ title: "Resource updated successfully." });
      } else {
        await resourcesService.create(values as any);
  ...
  return (
    <FormLayout
      ...
      description="Manage downloadable assets and resources."
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="Annual Report" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input {...field} placeholder="e.g. PDF, Video, Link" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
                ...
                render={({ field }) => (
                  <FormItem className="flex items-center justify-between rounded-lg border p-4">
                    <div className="space-y-0.5">
                      <FormLabel className="text-base">Gated Content</FormLabel>
                      <p className="text-sm text-muted-foreground">Requires email to download</p>
                    </div>
                    <FormControl>
